...
class Reboji {
    ...
    constructor(app) {
        ...
        app.post('/webhook/',  (req, res) => {
            ...
            for (let i = 0; i < messaging_events.length; i++) {
                ...
                if ( !this.isUserActive( facebookUserId ) ) {
                    this.getUserByFbId( facebookUserId )
                        .then( ( userObj )=> {
                            if ( !userObj ) {
                                // if user doesn't exist, create a user in the database
                                this.createUser( facebookUserId )
                            } else {
                                // already exists in the DB so simply add to active user list
                                this.addActiveUser( userObj )
                            }
                        })
                }
                ...
                if ( !this.currentPuzzle ) {
                    this.getUserCurrentPuzzle( facebookUserId )
                        .then( (userCurrentPuzzle) => {
                            if ( userCurrentPuzzle ) {
                                ...
                                // if there is no set user puzzle, get a random puzzle from the list
                                  = this.getPuzzleFromList()
                                // set the user's current puzzle to the randomly selected puzzle
                                this.setUserCurrentPuzzle(   )
                            }
                            // console.log ( 'webhook > getUserCurrentPuzzle: ',  , ' user puzzle', userCurrentPuzzle )
                            this.handleMessages(event, facebookUserId)
                        })
                } else {
                    this.handleMessages(event, facebookUserId)
                }
    ...
    addActiveUser( userObj ) {
        this.activeUsers.push( userObj )
    }

    isUserActive( userId ) {
        let r = !!this.activeUsers.filter( activeUserObj => activeUserObj.fbId == userId )[0]
        console.log ( 'isUserActive', r )
        return r
    }
    ...
    handleMessages( event ) {
        ...
        if (event.message && event.message.text) {
            ...
            if ( !  ) {
                ...
                setTimeout( ()=>this.sendTextMessage(  .pictogram ), this.firstMessageTime+(this.messageDelay*1) )
            }
            // help command
            else if ( text == "help" ) {
                this.sendTextMessage( '-' )
    ...
    getUserCurrentPuzzle( facebookUserId ) {
        return new Promise((resolve, reject) => {
